...
let x2 = $val1[3]
let y2 = $val1[4];
  return $KringleTuple([action, int(x1), int(y1), int(x2), int(y2)])
};


const parseLine = (line) => {
  return partsToTuple(captures("(toggle|turn on|turn off) (\\d+),(\\d+) through (\\d+),(\\d+)")(line))
};

let grid = $opStar([$opStar([0], 1000)], 1000)

for (let line of lines(input)) {
  let $val2 = [...parseLine(line)];
let action = $val2[0]
let x1 = $val2[1]
let y1 = $val2[2]
let x2 = $val2[3]
...
for (let col of range(x1)(x2)(1)) {
  
for (let row of range(y1)(y2)(1)) {
  if ($isEqual(action, "turn on")) {
  let $tmp3 = $subscript($subscript(grid, row), col)
$tmp3 = 1
$setSubscript($tmp3, $subscript(grid, row), col)

} else {
  if ($isEqual(action, "turn off")) {
  let $tmp4 = $subscript($subscript(grid, row), col)
$tmp4 = 0
$setSubscript($tmp4, $subscript(grid, row), col)

} else {
  let $tmp5 = $subscript($subscript(grid, row), col)
$tmp5 ^= 1
$setSubscript($tmp5, $subscript(grid, row), col)
